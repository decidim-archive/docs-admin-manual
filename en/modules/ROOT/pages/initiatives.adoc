...
`Collect participant personal data on signature` if ticked will enable a form (see <<personal-data-form-enabled>>) that users will have to fill in order to sign the initiative. The info gathered in that form will be exported along the signatures in the pdf export.

[#personal-data-form-enabled]
._Personal data collection form_.
image::image80.png[image]

`Enable promoting committee` if ticked and if minimum number of committee members is > 1 then initiative author will need to find at least 1 promoting committee member (as author they are automatically in it) to be able to send its initiative to technical validation.
...
- *SCOPES FOR THE INITIATIVE TYPE* need to be configured in hierarchical way following way (see <<child-scope-config>>) : 1 Parent - N Child
- `Only allow global scope initiative creation` is usually ticked in the *OPTIONS* section when the parent scope is the global scope.
- It works with an authorization handler that associates a scope to the user so make sure you select the right authorization handler in the *AUTHORIZATION SETTINGS* section (See <<authorization-handler-settings>>). (You can test it with the "Example authorization" `Dummy Authorization Handler`)
- /!\ this config option doesn't support offline votes yet, so make you selected *Online* as a `signature type` in the *OPTION* section.
...
`Only allow global scope initiatives creation`: you want to tick this flag if you ticked enable child scope (see <<global-parent-config>>) signature and configured the global scope as you parent scope. By enabling this initiative type selection will be skipped in the initiative creation wizard.
